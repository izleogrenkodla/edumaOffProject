<?php
global $post;
$item_visible    = !empty( $instance['visible'] ) ? $instance['visible'] : 2;
$pagination      = !empty( $instance['pagination'] ) ? $instance['pagination'] : 2;
$navigation      = !empty( $instance['navigation'] ) ? $instance['navigation'] : 2;
$list_cat        = !empty( $instance['cat_id'] ) ? $instance['cat_id'] : array();
$post_limit      = !empty( $instance['post_limit'] ) ? $instance['post_limit'] : 3;
$image_category  = get_template_directory_uri() . '/images/demo_images/demo_image_category.jpg';
...
if ( !empty( $list_cat ) ) {
	...
	foreach ( $list_cat as $k => $cat_id ) {
		...
		$top_image   = get_tax_meta( $cat_id, 'thim_archive_top_image', true );
		$description = term_description( $cat_id );

		$img = '<a href="' . esc_url( get_term_link( (int) $cat_id, $taxonomy ) ) . '">';
		...
		$html .= '<h3 class="title"><a href="' . esc_url( get_term_link( (int) $cat_id, $taxonomy ) ) . '">' . $cat_name . '</a></h3>';
